...
class BagOfEditsChangeEncoder(nn.Module):
    ...
    def __init__(self, token_embedding, vocab, **kwargs):
        super(BagOfEditsChangeEncoder, self).__init__()

        self.token_embedding = token_embedding
        self.vocab = vocab
        self.change_vector_size = token_embedding.size(1) * 2

    @property
    def device(self):
        return self.token_embedding.device
    ...
    def forward(self, code_changes, *args, **kwargs):
        ...
        changed_token_embeds = self.token_embedding[changed_token_ids]

        added_token_embeds = changed_token_embeds[:len(added_tokens)]
        deled_token_embeds = changed_token_embeds[len(added_tokens):]

        added_change_embeds = torch.zeros(len(code_changes), self.token_embedding.size(1), dtype=torch.float,
                                          device=self.device)
        ...
        deled_change_embeds = torch.zeros(len(code_changes), self.token_embedding.size(1), dtype=torch.float,
                                          device=self.device)
