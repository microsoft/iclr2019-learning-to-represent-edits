...
from diff_representation.asdl.transition_system.actions import ApplyRuleAction, ReduceAction, ApplySubTreeAction, \
    GenTokenAction
from diff_representation.asdl.transition_system.hypothesis import Hypothesis, SyntaxToken, TransitionSystem
from diff_representation.model import nn_utils
from diff_representation.model.sequential_decoder import SequentialDecoder, Decoder
...
class TransitionDecoder(Decoder):
    """tree-based decoder that constructs ASTs using a sequence of transition anctions"""
    ...
    def __init__(self, source_element_encoding_size, change_vector_size, hidden_size,
        ...
        if self.mode == 'tree2tree_subtree_copy':
                ...
                for batch_id, action_tm1 in enumerate(actions_tm1):
                    if action_tm1:
                        if isinstance(action_tm1, ApplyRuleAction):
                            ...
                            a_tm1_embed = self.syntax_tree_embedder.weight[self.vocab[action_tm1.token.value]]
    ...
    def forward(self, batch_examples, batched_prev_code, batched_context, change_vectors, embedding_cache=None, debug=False):
        ...
        # (tgt_action_len, batch_size, grammar_size + 1 + src_ast_node_num)f
        ...
        copy_prev_token_prob = self.syntax_token_copy_ptr_net(batched_prev_code.encoding,
                                                              batched_prev_code.syntax_token_mask if self.mode.startswith(
                                                                  'tree2tree') else batched_prev_code.mask,
                                                              att_vecs)
        ...
        if debug:
            return tgt_actions_prob, debug_info
        else:
            return tgt_actions_prob
    ...
    def run_syntax_token_rnn(self, examples, dev_init_vec=None, change_vectors=None):
        ...
        inputs = self.syntax_tree_embedder.get_embed_for_token_sequences(syntax_token_values)
    ...
    def syntax_token_rnn_step(self, tokens, h_tm1=None, change_vectors=None):
        ...
        if any(isinstance(x, list) for x in tokens):
            ...
            inputs = self.syntax_tree_embedder(torch.tensor(word_ids_sorted, dtype=torch.long, device=self.device))
            ...
            inputs = self.syntax_tree_embedder(torch.tensor(word_ids, dtype=torch.long, device=self.device))
    ...
    def get_gen_and_copy_index_and_mask(self, examples, batched_prev_code, batched_context,
        ...
        tgt_apply_rule_idx = torch.zeros(max_seq_len, batch_size, dtype=torch.long, device=self.device)
        tgt_apply_rule_mask = torch.zeros(max_seq_len, batch_size, dtype=torch.float, device=self.device)

        tgt_apply_subtree_idx = torch.zeros(max_seq_len, batch_size, dtype=torch.long, device=self.device)
        tgt_apply_subtree_mask = torch.zeros(max_seq_len, batch_size, dtype=torch.float, device=self.device)
        ...
        for batch_id in range(batch_size):
            prev_code = batched_prev_code.code_list[batch_id]
            context = batched_context.code_list[batch_id]
            ...
            for t, action_t in enumerate(tgt_actions):
                if isinstance(action_t, ApplyRuleAction):
                    app_rule_idx = self.grammar.prod2id[action_t.production]
                    tgt_apply_rule_idx[t, batch_id] = app_rule_idx
                    tgt_apply_rule_mask[t, batch_id] = 1
                elif isinstance(action_t, ApplySubTreeAction):
                    tgt_apply_subtree_idx[t, batch_id] = action_t.tree_node_id
                    tgt_apply_subtree_mask[t, batch_id] = 1
                elif isinstance(action_t, ReduceAction):
                    tgt_apply_rule_idx[t, batch_id] = len(self.grammar)
                    tgt_apply_rule_mask[t, batch_id] = 1
                else:
                    ...
                    if SequentialDecoder._can_only_generate_this_token(tgt_token.value):
                    ...
                    elif tgt_token.value in prev_code:
                        token_pos_list = [pos for pos, token in enumerate(prev_code) if token == tgt_token.value]
                        tgt_copy_prev_token_idx_mask[t, batch_id, token_pos_list] = 1
                        tgt_copy_prev_token_mask[t, batch_id] = 1
                    elif tgt_token.value in context:
                        ...
                        # we can only generate this code from the vocabulary
                        tgt_gen_token_mask[t, batch_id] = 1
                        tgt_gen_token_idx[t, batch_id] = self.vocab[tgt_token.value]

        return tgt_apply_rule_idx, tgt_apply_rule_mask, \
               tgt_apply_subtree_idx, tgt_apply_subtree_mask, \
               tgt_gen_token_idx, tgt_gen_token_mask, \
               tgt_copy_ctx_token_idx_mask, tgt_copy_ctx_token_mask, \
    ...
    def beam_search_with_source_encodings(self, prev_code, prev_code_encoding, context, context_encoding, change_vector,
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            if self.mode.startswith('tree2tree_subtree_copy'):
            ...
            for hyp_id, hyp in enumerate(hypotheses):
                ...
                for action_type in action_types:
                    if action_type == ApplyRuleAction:
                        ...
                        apply_rule_cand_mask[hyp_id, valid_cont_prod_ids] = 1
                    elif action_type == ApplySubTreeAction and t > 0 and self.mode.startswith('tree2tree_subtree_copy'):
            ...
            for hyp_id, hyp in enumerate(hypotheses):
                ...
                for action_type in action_types:
                    if action_type == ApplyRuleAction:
                        ...
                        if not self.mode.startswith('tree2tree_subtree_copy') or hyp.frontier_field is None:
                            continue
                        ...
                        for node_id, node in prev_code.descendant_nodes:
                            ...
                            if node.production.type in valid_continuating_types and not (node.production.type.name == 'IdentifierNameSyntax' and not self.copy_identifier):
            ...
            for new_hyp_idx, (new_hyp_score, new_hyp_flattened_pos) in enumerate(zip(top_new_hyp_scores, top_new_hyp_pos)):
                ...
                if new_hyp_flattened_pos < len(new_hyp_meta) and new_hyp_meta[new_hyp_flattened_pos]['action_type'] != 'gen_token':
                    ...
                    if prev_hyp.frontier_field.cardinality == 'single' and token == TransitionSystem.END_OF_SYNTAX_TOKEN_LIST_SYMBOL:
                        continue
...
class TransitionDecoderWithGraphEncoder(TransitionDecoder):
    def __init__(self, node_encoding_size, change_vector_size, hidden_size,
                 action_embed_size, field_embed_size,
                 dropout,
                 init_decode_vec_encoder_state_dropout,
                 syntax_tree_embedder,
                 vocab,
                 grammar,
                 mode,
        ...
        copy_identifier = kwargs.pop('copy_identifier', True)

        super(TransitionDecoderWithGraphEncoder, self).__init__(node_encoding_size, change_vector_size, hidden_size,
                                                                action_embed_size, field_embed_size,
                                                                dropout,
        ...
        self.copy_identifier = copy_identifier

        del self.decoder_cell_init
        self.decoder_cell_init = nn.Linear(node_encoding_size + change_vector_size, hidden_size)
