...
class SequentialDecoder(Decoder):
    ...
    @staticmethod
    def populate_gen_and_copy_index_and_mask(example, vocab, no_copy=False):
        prev_code = example.previous_code_chunk
        updated_code = example.updated_code_chunk
        context = example.context

        seq_len = len(example.updated_code_chunk) + 1
...
class SequentialDecoderWithTreeEncoder(SequentialDecoder):
    ...
    def get_gen_and_copy_index_and_mask(self, examples, batched_prev_code, batched_context):
        ...
        for batch_id in range(batch_size):
            updated_code = examples[batch_id].updated_code_chunk
            prev_ast = examples[batch_id].prev_code_ast
