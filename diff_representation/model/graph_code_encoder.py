...
from .encoder import *
from .gnn import AdjacencyList, GatedGraphNeuralNetwork
from .utils import get_method_args_dict


class GraphCodeEncoder(nn.Module):
    def __init__(self, hidden_size, syntax_tree_embedder, layer_timesteps, residual_connections, dropout, vocab, grammar, **kwargs):
        args = get_method_args_dict(self.__init__, locals())
        super(GraphCodeEncoder, self).__init__()

        self.args = args
        self.grammar = grammar
        self.vocab = vocab

        self.token_bidirectional_connection = kwargs.pop('token_bidirectional_connection', True)
        self.top_down_connection = kwargs.pop('top_down_connection', True)
        self.bottom_up_connection = kwargs.pop('bottom_up_connection', True)
        self.next_sibling_connection = kwargs.pop('next_sibling_connection', True)
        self.prev_sibling_connection = kwargs.pop('prev_sibling_connection', True)
    ...
    def forward(self, batch_syntax_trees, prev_code_token_encoding):
        ...
        # (V, D)
        init_node_embeddings = self.syntax_tree_embedder.embed_syntax_tree(batch_syntax_trees, batch_node2_example_node_map, prev_code_token_encoding)
        ...
        batch_node_encoding, batch_node_encoding_mask, batch_node_syntax_token_mask = self.to_batch_encoding(flattened_node_encodings, batch_syntax_trees, example_node2batch_node_map)

        return batch_node_encoding, batch_node_encoding_mask, batch_node_syntax_token_mask
    ...
    def to_batch_encoding(self, flattened_node_encodings, batch_syntax_trees, example_node2batch_node_map):
        ...
        batch_node_encoding_mask = torch.tensor(mask_list, dtype=torch.uint8, device=self.device)
        batch_node_syntax_token_mask = torch.tensor(mask_list, dtype=torch.uint8, device=self.device)
